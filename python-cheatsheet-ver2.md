...
# ─────────────────────────────────────────
# ⭐ 문자열 안에 따옴표를 넣어야 할 때
# 핵심 원칙: 바깥 따옴표 ≠ 안쪽 따옴표 → 충돌 없음
# ─────────────────────────────────────────

# 큰따옴표로 감싸면 → 안에 작은따옴표 자유롭게 사용 가능
print("I'm fine")          # 출력: I'm fine
print("It's a nice day")   # 출력: It's a nice day

# 작은따옴표로 감싸면 → 안에 큰따옴표 자유롭게 사용 가능
print('He said "hi"')      # 출력: He said "hi"
print('She said "hello"')  # 출력: She said "hello"

# ─────────────────────────────────────────
# ❗ 같은 종류 따옴표가 충돌하면? → 백슬래시(\)로 탈출
# \' 는 '문자열 끝'이 아니라 '작은따옴표 글자' 로 인식하게 함
# ─────────────────────────────────────────
print('I\'m fine')         # 출력: I'm fine    ← \' → '
print("He said \"hi\"")    # 출력: He said "hi" ← \" → "
...
# ⭐ 삼중따옴표(""") — 이럴 때 쓴다
#
# 1. 여러 줄 문자열을 그대로 쓰고 싶을 때
#    → 코드 안에 줄바꿈을 그냥 Enter로 넣을 수 있음
#    → \n 없이도 여러 줄 출력됨
#
# 2. 작은따옴표 + 큰따옴표가 동시에 들어갈 때
#    → 어느 쪽 탈출도 필요 없음
# ─────────────────────────────────────────
...
# ↑ 일반 따옴표로 쓰면? → 탈출 필요
print('It\'s a "test".')      # 출력: It's a "test".  (\'로 탈출)
```

## 0-2. 특수문자 출력 — 백슬래시(\) 탈출 규칙

```python
# 백슬래시(\)로 시작하는 특수문자 = "이스케이프 시퀀스"
# 두 글자처럼 보이지만 실제로는 하나의 기능을 하는 특수문자

# \n  → 줄바꿈 (n = newline)
# \t  → 탭     (t = tab)
# \\  → 백슬래시 자체 출력 (\ 하나만 쓰면 탈출 문자로 인식되므로 두 번 써야 함)
# \'  → 작은따옴표
# \"  → 큰따옴표
...
print("a\tb")         # 출력: a    b  ← \t 위치에 탭 간격 삽입
print("C:\\Users")    # 출력: C:\Users ← \\ → \ 하나로 출력

# ─────────────────────────────────────────
# 🔥 실전 예시: !@#$%^&*(\'"<>?:; 출력
#
# 이 문자열에는 \, ', " 가 동시에 들어 있어서
# 단순히 한 종류 따옴표로는 안 된다 → 3가지 해결법
# ─────────────────────────────────────────

# 방법1: 서로 다른 따옴표 두 덩어리로 이어붙이기
#        앞 덩어리: 작은따옴표 → "를 자유롭게 포함 가능
#        뒤 덩어리: 큰따옴표 → '를 자유롭게 포함 가능
print('!@#$%^&*(\\' + '\'"<>?:;')  # 출력: !@#$%^&*(\'"<>?:;

# 방법2: 백슬래시로 전부 탈출
print('!@#$%^&*(\\\'\"<>?:;')      # 출력: !@#$%^&*(\'"<>?:;

# 방법3 (가장 안전): 삼중따옴표 — 탈출 불필요
print("""!@#$%^&*(\'"<>?:;""")     # 출력: !@#$%^&*(\'"<>?:;
```
...
# 특징: 각 항목 사이에 자동으로 공백 1칸 삽입
#       숫자도 자동으로 str 변환 → 타입 오류 없음
# ─────────────────────────────────────────
print(a, "+", b, "=", a + b)    # 출력: 4 + 5 = 9

# sep 옵션 → 자동 공백 대신 원하는 구분자로 변경
print(a, b, sep=",")            # 출력: 4,5
print(a, b, sep="")             # 출력: 45   ← 구분자 없이 붙이기

# ─────────────────────────────────────────
# ✅ f-string — 가독성 최고, 실무/코테 모두 사용
# 특징: 문자열 앞에 f를 붙이고 {} 안에 변수나 식을 넣으면
#       실행 시점에 그 값으로 자동 교체됨
# ─────────────────────────────────────────
print(f"{a} + {b} = {a + b}")  # 출력: 4 + 5 = 9
#       ↑{a}→4  ↑{b}→5  ↑{a+b}→9

print(f"{a:.2f}")               # 출력: 4.00  (소수점 2자리 고정)
print(f"{a:05d}")               # 출력: 00004 (총 5자리, 빈 자리 0으로)

# ─────────────────────────────────────────
# ❌ + 연산 — 문자열끼리만 붙일 수 있음
# 숫자는 반드시 str()로 변환해야 함 → 실수 잦아서 코테 비추천
# ─────────────────────────────────────────
print("a = " + str(a))         # 출력: a = 4  ✅
print("a = " + a)              # ❌ TypeError: 문자열 + 숫자는 불가!

# ─────────────────────────────────────────
# end 옵션 — print의 마지막 문자 지정 (기본: '\n' 줄바꿈)
# ─────────────────────────────────────────
print("hello", end=" ")        # 줄바꿈 없이 공백으로 끝냄
print("world")                 # 출력: hello world  ← 한 줄로 이어짐
```
...
# ─────────────────────────────────────────
# 리스트 내용 출력 — *arr (언패킹)
# *arr : 리스트를 풀어서 각 원소를 개별 인자로 전달
# print(arr)   →  [1, 2, 3]  ← 대괄호 포함 출력 (보통 원치 않음)
# print(*arr)  →  1 2 3      ← 대괄호 없이 공백 구분 출력
# ─────────────────────────────────────────
arr = [1, 2, 3]
print(arr)                     # 출력: [1, 2, 3]  ← 리스트 그대로
print(*arr)                    # 출력: 1 2 3       ← 코테 정답 형식!
print(*arr, sep=",")           # 출력: 1,2,3
...
# 숫자 포맷
print(f"{3.14159:.2f}")        # 출력: 3.14       ← 소수점 2자리 (반올림)
print(f"{42:05d}")             # 출력: 00042      ← 총 5자리, 0으로 채움
print(f"{1000000:,}")          # 출력: 1,000,000  ← 천 단위 쉼표
```
...
이 한 줄이 코테에서 가장 많이 쓰이는 입력 패턴. **안에서 바깥 순서**로 읽는다.

```python
# 1단계: input()
# 사용자 입력을 통째로 하나의 문자열로 받음
# → "1 2 3 4 5" (문자열)

# 2단계: input().split()
# 공백 기준으로 문자열을 쪼개서 리스트로 만듦
# → ['1', '2', '3', '4', '5']  ⚠️ 아직 숫자가 아닌 문자열!
input().split()

# 3단계: map(int, ...)
# 리스트의 각 원소에 int() 함수를 적용 → 문자열을 숫자로 변환
# map 객체는 아직 리스트가 아님 (변환 준비만 된 상태)
map(int, ['1', '2', '3'])
...
# ⚠️ map 객체는 바로 출력하면 주소값이 나옴 → 반드시 list()로 감싸야 함
map(int, ['1', '2', '3'])      # 정수 변환
...
# 언패킹으로 바로 변수에 받기 (list() 없이도 OK)
a, b = map(int, input().split())       # 입력: "3 5" → a=3, b=5
n, m, k = map(int, input().split())   # 입력: "1 2 3" → n=1, m=2, k=3
```
...
s[0]        # 'a'  (첫 번째, 인덱스 0부터 시작)
s[-1]       # 'f'  (마지막, 음수 인덱스는 뒤에서부터)
s[1:4]      # 'bcd'  (인덱스 1 이상 4 미만)
s[:3]       # 'abc'  (처음~인덱스 2)
s[3:]       # 'def'  (인덱스 3~끝)
s[::-1]     # 'fedcba'  (역순 — step이 -1이면 뒤에서 앞으로)
s[::2]      # 'ace'  (2칸씩 건너뜀)

# ─────────────────────────────────────────
# 🔥 문자열은 immutable (변경 불가)
# s[2] = 'X'  → ❌ TypeError 발생!
# 수정하려면 → 앞뒤를 잘라서 새 문자열로 붙이는 방법 사용
# ─────────────────────────────────────────

# 삽입: 인덱스 i 위치에 new 삽입
result = s[:3] + "ZZZ" + s[3:]    # "abcZZZdef"

# 교체: 인덱스 i부터 len(replace)칸을 replace로 교체
replace = "ZZ"
i = 2
...
## 🔥 3-1. 값 순회 vs 인덱스 순회 — 코테 핵심 선택 기준

> 이걸 잘못 선택하면 풀 수 있는 문제도 틀린다. 반드시 구분하자.

```python
arr = ["a", "b", "c", "d"]   # 또는 문자열 s = "abcd"
...
# ─────────────────────────────────────────
# 🅱 인덱스 순회: for i in range(len(arr))
# → i에 0, 1, 2, 3 ... 인덱스 숫자가 들어옴
# → arr[i]로 원소 접근
# ─────────────────────────────────────────
for i in range(len(arr)):
    print(i, arr[i])    # 0 a, 1 b, 2 c, 3 d

# ✅ 이럴 때 써라:
# "짝수 인덱스의 원소만 모아라" → 인덱스 홀짝이 중요
# "현재 위치(idx)가 조건에 영향을 준다" → 인덱스 필요
for i in range(len(arr)):
    if i % 2 == 0:          # ← 인덱스를 조건에 써야 할 때
        result += arr[i]
...
# 실수: 인덱스 순회가 필요한데 값 순회를 씀
s = "abc123"
for c in s:           # c는 문자 'a','b','c'...
    if c % 2 == 0:    # ❌ 문자에 % 연산 불가 → TypeError
        ...

# ✅ 해결: 인덱스 순회로 바꿈
for i in range(len(s)):   # i는 0,1,2,3...
    if i % 2 == 0:        # ✅ 인덱스로 조건 판별
        result += s[i]    # ✅ s[i]로 실제 문자 접근


# ─────────────────────────────────────────
# 🔥 선택 기준 한 줄 요약
# ─────────────────────────────────────────
# "원소 값 자체만 쓸 거면"       → for x in arr
# "위치(인덱스)가 조건에 필요하면" → for i in range(len(arr))
# "둘 다 필요하면"               → for i, v in enumerate(arr)
```
...
# ❌ 실수: elif에 조건 없이 대입식을 씀
elif answer = a + b + c:   # ❌ SyntaxError! elif는 조건이 들어와야 함

# ✅ elif는 반드시 True/False가 되는 "조건식"이어야 함
elif a == b:               # ✅ 비교 연산
elif x > 0:               # ✅ 대소 비교
elif not flag:            # ✅ 논리식

# ─────────────────────────────────────────
# 🔥 True/False 비교 생략 — Python 스타일
# ─────────────────────────────────────────

# ❌ 불필요하게 길게 쓰는 경우
if included[i] == True:    # ❌ 의미상 맞지만 Python 스타일 아님

# ✅ 그냥 변수 자체가 조건
if included[i]:            # ✅ True/False 자체를 조건으로 사용
if not included[i]:        # ✅ False일 때
...
# 🔥 4-5. 변수 vs 함수 — 가장 많이 하는 실수 🔥

> 초보자가 가장 자주 하는 실수. `변수이름(값)` 은 **함수 호출**로 해석된다.
...
result = ""         # result는 문자열 변수

result(i)           # ❌ Python 해석: "result라는 함수를 호출해라"
                    # 하지만 result = "" → 문자열! 함수가 아님
                    # → TypeError: 'str' object is not callable

my_list = []
my_list(i)          # ❌ 리스트 변수도 마찬가지 → TypeError

included = [True, False, True]
included(0)         # ❌ → TypeError ([]로 접근해야 함)
...
# 1. 리스트/문자열 → 인덱스로 접근할 때는 []
included[0]         # ✅ → True
my_list[i]          # ✅ → i번째 원소

# 2. 문자열에 값 이어붙일 때는 +=
result += str(i)    # ✅ → i를 문자열로 변환해서 이어붙임

# 3. 리스트에 원소 추가할 때는 .append()
my_list.append(i)   # ✅
...
# ─────────────────────────────────────────
#
# 함수/메서드 호출    → ()   예: print(), len(), str()
# 리스트/문자열 접근  → []   예: arr[i], s[0], included[i]
# 딕셔너리 접근      → []   예: d["key"], d[0]
#
# 즉: 변수 뒤에 (값) 형태가 나오면 → 내가 함수 호출을 하려는 건지 확인!
...
# 문자열 누적 (코테 최빈출)
odd_list = ""
odd_list += str(i)          # ✅ i를 문자열로 바꿔서 이어붙임

# join 패턴 (더 깔끔)
''.join(str(i) for i in arr if i % 2 == 1)   # ✅

# 숫자 누적
answer = 0
answer += (a + i * d)       # ✅

# 곱 누적 → 초기값 반드시 1 (0이면 영원히 0)
product = 1                 # ✅ (0이면 ❌)
product *= i
```
...
s = "10110"
c = s[0]        # c = '1'  → 문자 '1' (숫자 1이 아님!)

# ❌ 문자를 숫자처럼 비교
if s[i] == 1:   # ❌ '1' != 1  → 항상 False → 로직 버그!
...
# 문자 → 숫자
int("1")        # 1
int(s[i])       # s[i]가 숫자 문자일 때 정수로 변환

# 숫자 → 문자
str(1)          # "1"
str(i)          # i를 문자열로

# 문자열 → 정수 (이어붙인 결과를 계산할 때)
int("351")      # 351
...
# 문제가 정수 반환을 요구하면 → 문자열로 return하면 오답
if product < sum_num * sum_num:
    return "1"  # ❌ 문자열 "1" 반환
    return 1    # ✅ 정수 1 반환
```
...
🔴 문법 실수 (코드가 아예 실행 안 됨)
──────────────────────────────────────────────────────────
1.  : 붙였는가? (if, for, def, while, elif, else 전부!)
2.  == 썼는가? (조건에서 = 하나만 쓰면 SyntaxError)
3.  True/False 대문자인가? (true/false는 에러)
4.  들여쓰기 일관성 → if/for 블록 안 코드가 들여쓰기 됐는가?
5.  elif는 "조건"이 들어와야 함 (elif answer = x 는 SyntaxError)
6.  곱 누적 초기값: product = 1 (0이면 영원히 0!)

──────────────────────────────────────────────────────────
🔴 변수 vs 함수 실수 (런타임 에러)
──────────────────────────────────────────────────────────
7.  변수(값) 형태 쓰면 함수 호출로 해석 → TypeError
    result(i) ❌  →  result += str(i) ✅
    included(i) ❌  →  included[i] ✅ (리스트 접근은 [])

──────────────────────────────────────────────────────────
🔴 순회 방식 실수 (로직 버그)
──────────────────────────────────────────────────────────
8.  인덱스가 조건에 필요한데 값 순회를 씀
    for c in s → c는 문자, 인덱스 % 2 못 씀
    for i in range(len(s)) → i는 인덱스 ✅
9.  인덱스 순회 후 원소 접근: arr[i] (arr 자체가 아님!)

──────────────────────────────────────────────────────────
🔴 타입 실수 (로직 버그 — 에러 안 나서 더 위험)
──────────────────────────────────────────────────────────
10. 문자열에서 꺼낸 건 문자: s[i] == 1 ❌  →  s[i] == "1" ✅
11. True/False 비교 불필요: included[i] == True ❌  →  included[i] ✅
12. return 타입: return "1" ❌  →  return 1 ✅ (정수 요구 시)
13. 문자열 + 숫자 → str() 변환 필수 ("값: " + str(n))
14. join 안에 숫자 → str 변환 후 (''.join(map(str, arr)))

──────────────────────────────────────────────────────────
🔴 자료구조 실수
──────────────────────────────────────────────────────────
15. 인덱스 범위 초과 → IndexError 가능성 체크
16. 2D 배열 [[0]*m]*n ❌ (같은 행 공유!) → [[0]*m for _ in range(n)]
17. sort()는 원본 변경·반환 None, sorted()는 새 리스트
18. arr.remove(x)는 첫 번째 하나만 제거
19. deque 대신 list.pop(0) 쓰면 O(n)으로 느림

──────────────────────────────────────────────────────────
🔴 기타 실수
──────────────────────────────────────────────────────────
20. 재귀 깊이 기본 1000 → setrecursionlimit 필요 시
21. random.shuffle() 반환값 없음 → 원본 수정됨
22. timedelta 차이 → .days로 꺼내야 숫자 사용 가능
23. sys.stdin.readline 쓸 때 줄 끝 \n → strip() 필수
24. str.find()는 없으면 -1, str.index()는 없으면 ValueError
25. Counter는 없는 키도 0 반환, dict는 KeyError
```
