...
# def solution(spell, dic):
#     new_array = list(combinations(spell,1))
#     answer = 0
#     return new_array
...
# def solution(spell, dic):
#     new_array = []
#     new_array.append(list(combinations(spell,3)))

#     print(new_array)


# 힌트: 반대로 생각해보기
# spell = ["z", "d", "x"], dic에 "dzx"가 있는 경우를 보면
# "dzx"가 spell로 만들 수 있는 단어인지 어떻게 확인할까요
# 이 내용 gpt에 힌트 검색함

# 실행한 결괏값 2이 기댓값 1과 다릅니다.
# def solution(spell, dic):
#     for word in dic:
#         if sorted(word) == sorted(spell):
#             return 1
...
# ["def", "dww", "dzx", "loveaw"] 순서대로 보면
# "def" 확인 → spell이랑 다름 → 바로 return 2 ❌
# "dww" 확인 (X, 이미 끝남)
# "dzx" 확인 (X, 이미 끝남)  ← 여기까지 와야 1인데!
# 첫 번째 단어에서 바로 return 2를 해버려서 "dzx"까지 못 가는 거예요.
# return 2는 모든 단어를 다 확인한 후에 내려야 해요.